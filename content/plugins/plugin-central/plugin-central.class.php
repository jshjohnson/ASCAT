...
class PluginCentral
{
    ...
    function row($file, $p, $update, $response)
    {
        ...
        {
			//CyberOto tweak
            printf(__('<li>%1$s %3$s <a href="%2$s">download</a> <em>(automatic update unavailable)</em>. <a href="javascript:verifyPlugin(\'%4$s\', \'Are you sure you want to ignore future updates of this plugin?\')">Ignore</a></li>'), $p['Name'], $r->url, $r->new_version, urlencode("index.php?action=pc_ignore&file=" . $file . "&name=" . $p["Name"]));
			//CyberOto tweak end
        }
        else 
        {
