...
if ( !class_exists( 'WPSL_Admin' ) ) {
	...
	class WPSL_Admin extends WP_Store_locator {
		...
		public function create_admin_menu() {	
			$hook = add_menu_page( 'Store Locator', __( 'Centre Locator', 'wpsl' ), apply_filters( 'wpsl_capability', 'manage_options' ), 'wpsl_store_editor', array( $this, 'manage_stores' ), plugins_url( 'img/store-locator-icon.png', dirname( __FILE__ ) ), apply_filters( 'wpsl_menu_position', null ) );
            add_submenu_page( 'wpsl_store_editor', __( 'Manage Centres', 'wpsl' ), __( 'Manage Centres', 'wpsl' ), apply_filters( 'wpsl_capability', 'manage_options' ), 'wpsl_store_editor', array( $this, 'manage_stores' ) );
			add_submenu_page( 'wpsl_store_editor', __( 'Add Centre', 'wpsl' ), __( 'Add Centre', 'wpsl' ), apply_filters( 'wpsl_capability', 'manage_options' ), 'wpsl_add_store', array( $this, 'add_store' ) );
		...
		public function add_new_store() {
            ...
            if ( $result === false ) {
                $state = 'error';
                $msg = __( 'There was a problem saving the new centre details, please try again.', 'wpsl' );
            } else {
                $_POST = array();
                $state = 'updated';
                $msg = __( 'Centre succesfully added.', 'wpsl' );
            } 
        ...
        public function create_menu() {
            ...
            $menu_items = array(
                'wpsl_store_editor' => __( 'Current Centres', 'wpsl' ),
                'wpsl_add_store'    => __( 'Add Centre', 'wpsl' ),
